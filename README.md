### Hey, I’m Dobrynya Avdyushin

**Founder & Head of Engineering at [Avdyushin Consulting Group](https://avdyushin.ru) · Backend & Trading Systems · FinTech / Crypto Infrastructure**

I build backend systems, trading tools, and engineering infrastructure for fintech and crypto products - with a focus on reliability, performance, automation, and clean developer experience.

Currently building systems for **cross-exchange trading**, **funding-rate arbitrage**, backend services, and internal tooling.

---

### What I Do

* Build backend services and APIs with **Go**, **TypeScript**, and **Rust**
* Design tools for **crypto trading infrastructure** and market automation
* Work with databases, containers, monitoring, deployment, and Linux-based environments
* Prototype products from idea to working MVP
* Lead engineering decisions across architecture, stack, and delivery

---

### Core Stack

![Go](https://img.shields.io/badge/Go-Backend-00ADD8?style=flat-square\&logo=go\&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-Fullstack-3178C6?style=flat-square\&logo=typescript\&logoColor=white)
![Rust](https://img.shields.io/badge/Rust-Systems-000000?style=flat-square\&logo=rust\&logoColor=white)
![React](https://img.shields.io/badge/React-Frontend-61DAFB?style=flat-square\&logo=react\&logoColor=black)
![Python](https://img.shields.io/badge/Python-Automation-3776AB?style=flat-square\&logo=python\&logoColor=white)

---

### Infrastructure & Tools

![Linux](https://img.shields.io/badge/Linux-Daily%20Driver-FCC624?style=flat-square\&logo=linux\&logoColor=black)
![Docker](https://img.shields.io/badge/Docker%20%2F%20Compose-Containers-2496ED?style=flat-square\&logo=docker\&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-Database-4169E1?style=flat-square\&logo=postgresql\&logoColor=white)
![Bash](https://img.shields.io/badge/Bash-Scripting-4EAA25?style=flat-square\&logo=gnubash\&logoColor=white)
![Figma](https://img.shields.io/badge/Figma-Product%20Design-F24E1E?style=flat-square\&logo=figma\&logoColor=white)

---

<img align="right" src="https://github-readme-stats-sumanth-talluri.vercel.app/api?username=avdushin&show_icons=true&title_color=CDA571&icon_color=CDA571&text_color=fff&bg_color=004242" />

### Areas I’m Interested In

* FinTech and crypto trading systems
* Cross-exchange market infrastructure
* Funding-rate arbitrage
* High-performance backend engineering
* Linux, developer tooling, and keyboard-driven workflows
* Split keyboards and ergonomic hardware

---

### Featured Project

#### K:02 Split Keyboard Build

I’m interested in how split keyboards change the way people work, think, and interact with computers.

My build: [keyboard_layout](https://github.com/Avdushin/keyboard_layout)

---

<!-- ### Code Practice

![](https://www.codewars.com/users/B3aver1/badges/large)

--- -->

### Contact

[Website](https://avdyushin.ru) · [Telegram](https://t.me/itdobro) · [Habr](https://habr.com/ru/users/Avdushin/) · [LinkedIn](https://www.linkedin.com/in/dobrynya-avdyushin) · [Twitter / X](https://twitter.com/ANAN4ZZ)
